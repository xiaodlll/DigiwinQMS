...
namespace Meiam.System.Interfaces
{
    ...
    public class BaseService<T> : IBaseService<T> where T : class, new()
    {
        ...
        public IEnumerable<T> GetRandomData(Expression<Func<T, bool>> where, int n) {
            var random = new Random();
            var shuffledData = Db.Queryable<T>().Where(where).OrderBy("NEWID()").ToList();
            return shuffledData.Take(n);
        }

        public DataTable ToDataTable<U>(IEnumerable<U> entities)
        {
            // 创建 DataTable 实例
            DataTable dataTable = new DataTable(typeof(U).Name);
            // 获取实体的所有属性
            ...
            ColumnMapping(dataTable, typeof(U));
            return dataTable;
        }
        ...
        /// <returns></returns>
        private void ColumnMapping(DataTable dt, Type type)
        {
            foreach (DataColumn column in dt.Columns)
            {
                string colName = column.ColumnName;
                string colCaption = GetPropertyDisplayName(type, colName);
                bool isRequired = GetPropertyRequired(type, colName);
                column.ColumnName = colCaption + (isRequired ? "*" : "");
            }
        }
        ...
        private string GetPropertyDisplayName(Type type, string propertyName)
        {
            ...
            // 返回 Display 的 Name 值
            return displayAttribute?.Name ?? propertyName; // 如果没有 Display 特性，返回属性名
        }

        private bool GetPropertyRequired(Type type, string propertyName)
        {
            // 获取属性
            var property = type.GetProperty(propertyName);
            if (property == null)
